...
class LightFieldButton(sg.Button):
    ...
    def __init__(self, size : tuple[int, int], position : tuple[int, int], light_on = False):
        """
        ...
        light_on : bool, optional
            Property if the light should be on or off effects the image which will be shown, by default False
        """
    ...
    def set_light_on(self, light_on : bool) -> None:
        """
        Sets the light_on status of this button and updates the image accordingly.

        Parameters
        ----------
        light_on : bool
            True if light should set to on else false.
        """
        self._light_on = light_on
        button_image = self._get_sized_button_image()
    ...
    def toggle_light(self) -> None:
        """
        Toggles the buttons light_on status. Always changes it to the opposite of what it was before and updates the image.
        """
        self.set_light_on(not self._light_on)
        
    def resize_base64_image(image64 : bytes, size : tuple[int, int]) -> bytes:
        """
        Private helper method to resize a base64 encoded image to a specific size.
    ...
    def _get_sized_button_image(self) -> None:
        ...
        """
        image = LightFieldButton.light_on_base64 if self._light_on else LightFieldButton.light_off_base64
